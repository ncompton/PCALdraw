 ...
 * @version 061714
 */
public class MyH6D implements EvioWritableTree {
	...
	private Axis bAxis = new Axis();
	private Axis cAxis = new Axis();
	...
	public MyH6D() {
		offset = new MyMultiIndex(xAxis.getNBins(), yAxis.getNBins(), zAxis.getNBins(),
				                aAxis.getNBins(), bAxis.getNBins(), cAxis.getNBins());
		hBuffer = new ArrayList<Double>(offset.getArraySize());
                ...
                this.attr.getProperties().setProperty("btitle", "");
                this.attr.getProperties().setProperty("ctitle", "");
    }
	 ...
	 */
	public MyH6D(String name, int bx, double xmin, double xmax, 
			                  int by, double ymin, double ymax,
			                  int bz, double zmin, double zmax,
			                  int ba, double amin, double amax, 
			                  int bb, double bmin, double bmax,
			                  int bc, double cmin, double cmax) {
		hName = name;
		this.set(bx, xmin, xmax, by, ymin, ymax, bz, zmin, zmax,
				 ba, amin, amax, bb, bmin, bmax, bc, cmin, cmax);
    ...
    public MyH6D(String name, String title, int bx, double xmin, double xmax, 
    									    int by, double ymin, double ymax,
    									    int bz, double zmin, double zmax,
    						                int ba, double amin, double amax, 
    						                int bb, double bmin, double bmax,
	...
	public final void set(int bx, double xmin, double xmax, 
		...
		bAxis.set(bb, bmin, bmax);
		cAxis.set(bc, cmin, cmax);
		offset = new MyMultiIndex(bx, by, bz, ba, bb, bc);
	 ...
	 */
	private boolean isValidBins(int bx, int by, int bz, int ba, int bb, int bc) {
		if ((bx >= 0) && (bx < xAxis.getNBins()) 
		 && (by >= 0) && (by < yAxis.getNBins())
		 && (bz >= 0) && (bz < zAxis.getNBins())
		 && (ba >= 0) && (ba < aAxis.getNBins())
		 && (bb >= 0) && (bb < bAxis.getNBins())
		 && (bc >= 0) && (bc < cAxis.getNBins())){
			return true;
		}
		return false;
	}
	 ...
	 */
	public double getBinContent(int bx, int by, int bz, int ba, int bb, int bc) {
		if (this.isValidBins(bx, by, bz, ba, bb, bc)) {
			int buff = offset.getArrayIndex(bx, by, bz, ba, bb, bc);
	...
	*/
	public final void setBTitle(String bTitle) {
		//this.getYaxis().setTitle(bTitle);
		this.attr.getProperties().setProperty("btitle", bTitle);
	}
	...
	*/
	public final void setCTitle(String cTitle) {
		//this.getYaxis().setTitle(zTitle);
		this.attr.getProperties().setProperty("ctitle",cTitle);
	}
	...
	*/
	public String getBTitle() {
		return this.attr.getProperties().getProperty("btitle", "");
		//return this.getBaxis().getTitle();
	}
	...
	*/
	public String getCTitle() {
		return this.attr.getProperties().getProperty("ctitle", "");
		//return this.getCaxis().getTitle();
	}
	...
	*/
	public void setBinContent(int bx, int by, int bz, int ba, int bb, int bc, double w) {
		if (this.isValidBins(bx, by, bz, ba, bb, bc)) {
			int buff = offset.getArrayIndex(bx, by, bz, ba, bb, bc);
			hBuffer.set(buff, w);
		}
	 ...
	 */
	public void fill(double x, double y, double z, double a, double b, double c) {
		int bin = this.findBin(x, y, z, a, b, c);
		if (bin >= 0)
			this.addBinContent(bin);
	}

	public void fill(double x, double y, double z, double a, double b, double c, double w) {
		int bin = this.findBin(x, y, z, a, b, c);
		if (bin >= 0) {
	 ...
	 */
	public int findBin(double x, double y, double z, double a, double b, double c) {
		int bx = xAxis.getBin(x);
		int by = yAxis.getBin(y);
		int bz = zAxis.getBin(z);
		int ba = aAxis.getBin(a);
		int bb = bAxis.getBin(b);
		int bc = cAxis.getBin(c);
		if (this.isValidBins(bx, by, bz, ba, bb, bc)) {
			return (offset.getArrayIndex(bx, by, bz, ba, bb, bc));
		}
		else
		{
	...
	public double[][][][][][] getContentBuffer() {
		double[][][][][][] buff = new double[xAxis.getNBins()][yAxis.getNBins()][zAxis.getNBins()][aAxis.getNBins()][bAxis.getNBins()][cAxis.getNBins()];
		for (int xloop = 0; xloop < xAxis.getNBins(); xloop++) {
			for (int yloop = 0; yloop < yAxis.getNBins(); yloop++) {
				for (int zloop = 0; zloop < zAxis.getNBins(); zloop++) {
					for (int aloop = 0; aloop < aAxis.getNBins(); aloop++) {
						for (int bloop = 0; bloop < bAxis.getNBins(); bloop++) {
							for (int cloop = 0; cloop < cAxis.getNBins(); cloop++) {
								buff[xloop][yloop][zloop][aloop][bloop][cloop] = this.getBinContent(xloop, yloop, zloop, aloop, bloop, cloop);
							}
						}
					}
				}
			...
			for (int yloop = 0; yloop < yAxis.getNBins(); yloop++) {
				for (int zloop = 0; zloop < zAxis.getNBins(); zloop++) {
					for (int aloop = 0; aloop < aAxis.getNBins(); aloop++) {
						for (int bloop = 0; bloop < bAxis.getNBins(); bloop++) {
							for (int cloop = 0; cloop < cAxis.getNBins(); cloop++) {
								buff[xloop][yloop][zloop][aloop][bloop][cloop] = 0.0;
							}
						}
					}
				}
			...
			for (int y = 0; y < yAxis.getNBins(); y++) {
				for (int z = 0; z < zAxis.getNBins(); z++) {
					for (int a = 0; a < aAxis.getNBins(); a++) {
						for (int b = 0; b < bAxis.getNBins(); b++) {
							for (int c = 0; c < cAxis.getNBins(); c++) {
								height += this.getBinContent(x, y, z, a, b, c);
							}
						}
					}
				}
	...
	public MyH1D projectionX(String name, int bymin, int bymax, int bzmin, int bzmax,
			                              int bamin, int bamax, int bbmin, int bbmax, 
			                              int bcmin, int bcmax) {
		//String name = "X Projection";
		double xMin = xAxis.min();
		double xMax = xAxis.max();
		...
		if(bbmin < 0) bbmin = 0;
		if(bbmax >= bAxis.getNBins()) bbmax = bAxis.getNBins() - 1;
		if(bcmin < 0) bcmin = 0;
		if(bcmax >= cAxis.getNBins()) bcmax = cAxis.getNBins() - 1;
			...
			for (int y = bymin; y <= bymax; y++) {
				for (int z = bzmin; z <= bzmax; z++) {
					for (int a = bamin; a <= bamax; a++) {
						for (int b = bbmin; b <= bbmax; b++) {
							for (int c = bcmin; c <= bcmax; c++) {
								height += this.getBinContent(x, y, z, a, b, c);
							}
						}
					}
...
*/
	...
	*/
    ...
    public TreeMap<Integer, Object> toTreeMap() {
        ...
        hcontainer.put(3, new int[]{this.getXAxis().getNBins(),this.getYAxis().getNBins(),this.getZAxis().getNBins(),this.getAAxis().getNBins(),this.getBAxis().getNBins(),this.getCAxis().getNBins()});
        hcontainer.put(4, new double[]{
            ...
            this.getBAxis().min(),this.getBAxis().max(),
            this.getCAxis().min(),this.getCAxis().max(),
        });
        ...
        if(map.get(1) instanceof int[]){
            if(  ((int[]) map.get(1))[0]==3){
                ...
                this.set(nbins[0], binsrange[0],binsrange[1],
                		 ...
                		 nbins[4], binsrange[8],binsrange[9],
                		 nbins[5], binsrange[10],binsrange[11]);
    ...
    public Double getData(int x, int y, int z, int a, int b, int c) {
        return this.getBinContent(x, y, z, a, b, c);
    }
    ...
    public Integer getDataSize(int axis) {
        ...
        if(axis==4) return this.getBAxis().getNBins();
        if(axis==5) return this.getCAxis().getNBins();
        return 0;
    }
	
	public static void main(String[] args){ 
		MyH6D testh3 = new MyH6D("test1", 500, 0.0, 300.0, 
					                      1, 0.0, 300.0,
					                      1, 0.0, 300.0,
					                      68, 0.5, 68.5,
					                      62, 0.5, 62.5,
		...
		testh3.fill(0.5, 0.5, 0.5, 0.7, 0.7, 0.7);
		testh3.fill(0.5, 0.5, 0.5, 0.7, 0.7, 0.7);
		testh3.fill(1.5, 1.5, 1.5, 1.5, 1.5, 1.5);
		
		testh3.fill(4.5, 4.5, 4.5, 4.5, 4.5, 4.5);
		testh3.fill(0.1, 4.5, 2.5, 0.6, 4.5, 2.5);
		
		//System.out.println("bin: " + 0 + " content: " + testh3.getBinContent(0, 0, 0, 0, 0, 0));
		...
		testh1 = testh3.projectionX("test",0,1,0,2,0,2,0,2,0,2).histClone("newtest");
		//testh1.fill(5.0);
